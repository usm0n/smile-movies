...
function Search() {
  ...
  const totalPages = type === "movie"
    ...
    : type === "tv" ? (searchTvData?.data as searchTV)?.total_pages :
      Math.max((searchMovieData?.data as searchMovie)?.total_pages || 0, (searchTvData?.data as searchTV)?.total_pages || 0);

  const totalResults = type === "all"
    ? ((searchMovieData?.data as searchMovie)?.total_results || 0) + ((searchTvData?.data as searchTV)?.total_results || 0)
    : type === "movie"
  ...
  return (
    <Box sx={{ width: "90%", padding: "100px 0px", margin: "0 auto", display: "flex", flexDirection: "column", gap: "40px", minHeight: "100vh" }}>
      <Typography level="h1" fontWeight={700} sx={{ "@media (max-width: 768px)": { fontSize: "30px" } }}>
        Search Results for: <Typography fontWeight={400} textColor={"neutral.300"}>{query}</Typography>
      </Typography>

      <ButtonGroup>
        <IconButton onClick={() => setType("all")} color={type === "all" ? "primary" : "neutral"} variant={type === "all" ? "solid" : "outlined"} sx={{ padding: "0 20px" }}>All</IconButton>
        <IconButton onClick={() => setType("movie")} color={type === "movie" ? "primary" : "neutral"} variant={type === "movie" ? "solid" : "outlined"} sx={{ padding: "0 20px" }}>Movies</IconButton>
        <IconButton onClick={() => setType("tv")} color={type === "tv" ? "primary" : "neutral"} variant={type === "tv" ? "solid" : "outlined"} sx={{ padding: "0 20px" }}>TV shows</IconButton>
      </ButtonGroup>
