import { Box, Skeleton, Tooltip, Typography, Button, Chip } from "@mui/joy";
...
// ─── Colour system (matches seriesgraph style) ─────────────────────────────
...
const TIER_COLORS: Record<Tier, { bg: string; text: string; border: string }> = {
  ...
  none:     { bg: "rgb(189, 189, 189)",  text: "#444466", border: "#2a2a4a" },
};
...
interface SeasonData {
  seasonNumber: number;
  episodes: Map<number, ImdbEpisode>; // keyed by episodeNumber
  average: number;
}
...
function TVRatings() {
  ...
  useEffect(() => {
    ...
    const fetchAll = async () => {
      ...
      try {
        ...
        // Step 2: fetch episodes per season in parallel
        ...
        setSeasons(rawSeasons.filter((s) => s.episodes.size > 0));
      } catch {
        setError("Failed to load IMDb ratings.");
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    ...
    >
        ...
        <Box>
          ...
          {tvData && (
            <Typography level="body-md" textColor="neutral.400">
              {tvData.name} · {seasons.length} seasons
            </Typography>
          )}
        </Box>
      </Box>
        ...
        {LEGEND.map((item) => {
          ...
          return (
            <Box key={item.tier} sx={{ display: "flex", alignItems: "center", gap: 0.5 }}>
              ...
              />
              <Typography level="body-xs" sx={{ color: c.text, whiteSpace: "nowrap" }}>
                {item.label}
              </Typography>
      ...
      {/* ── Error ── */}
      {error && (
        <Typography level="body-md" textColor="danger.400" sx={{ mb: 3 }}>
          {error}
        </Typography>
      )}
          ...
          <Box sx={{ display: "inline-grid", gap: 1 }}>
            {Array(8).fill(null).map((_, i) => (
              <Box key={i} sx={{ display: "flex", gap: 1 }}>
                <Skeleton variant="rectangular" width={36} height={36} sx={{ borderRadius: "6px", flexShrink: 0 }} />
                {Array(5).fill(null).map((_, j) => (
                  <Skeleton key={j} variant="rectangular" width={54} height={36} sx={{ borderRadius: "6px" }} />
                ))}
              </Box>
            ))}
          </Box>
      ...
      {!loading && !error && seasons.length > 0 && (
        <Box sx={{ overflowX: "auto", pb: 2 }}>
          ...
          >
            {/* ── Season header row ── */}
            <Box sx={{ display: "flex", gap: "4px", mb: "4px", pl: "44px" }}>
              {seasons.map((s) => (
                <Box
                  key={s.seasonNumber}
                  sx={{
                ...
                >
                  <Typography
                    level="body-xs"
                    sx={{ fontWeight: 700, color: "rgba(255,255,255,0.7)", fontSize: 11 }}
                  >
                    S{s.seasonNumber}
                  </Typography>
                </Box>
            ...
            {Array.from({ length: maxEpisodes }, (_, i) => i + 1).map((epNum) => (
              <Box key={epNum} sx={{ display: "flex", gap: "8px", mb: "4px", alignItems: "center" }}>
                ...
                <Box sx={{ width: 40, flexShrink: 0, textAlign: "right", pr: "4px" }}>
                  <Typography
                    level="body-xs"
                    sx={{ fontSize: 10, color: "rgba(255,255,255,0.4)", fontWeight: 600 }}
                  >
                    E{epNum}
                  </Typography>
                </Box>
                ...
                {seasons.map((s) => {
                  ...
                  const isHovered =
                    hoveredCell?.s === s.seasonNumber && hoveredCell?.e === epNum;
                  ...
                  return (
                    ...
                    >
                      <Box
                        onClick={() =>
                          ep
                            ? navigate(`/tv/${tvId}/${s.seasonNumber}/${epNum}/watch`)
                            : undefined
                        }
                        onMouseEnter={() =>
                          ep && setHoveredCell({ s: s.seasonNumber, e: epNum })
                        }
                        ...
                        sx={{
                          ...
                          // border: `1px solid ${ep ? c.border : "rgba(255,255,255,0.06)"}`,
                          display: "flex",
                          alignItems: "center",
            ...
            <Box sx={{ display: "flex", gap: "8px", mt: "8px", pl: "48px", alignItems: "center" }}>
              {seasons.map((s) => {
                ...
                return (
                  <Box
                    key={s.seasonNumber}
                    sx={{ width: 50, flexShrink: 0, textAlign: "center" }}
                  >
                    <Chip
                      size="sm"
                      sx={{
            ...
            <Box sx={{ pl: "44px", mt: "2px" }}>
              <Typography level="body-xs" textColor="neutral.500" sx={{ fontSize: 10 }}>
                AVG.
              </Typography>
            </Box>
          </Box>
